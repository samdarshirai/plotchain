...
// @MockBean on the repository INTERFACE so this runs a real PayoutBankAccountService inside a
// real Spring Security filter chain, per CompanyBrandingControllerTest's pattern.
// PayoutBankAccountService now depends on the real SettingsAuditService bean, so its own
