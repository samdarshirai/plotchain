...
public interface AssociateRepository extends JpaRepository<Associate, UUID> {
    ...
    // Dashboard's per-leg associate counts (Business Volume / Network & Income tiles): identical
    // shape to countDownline above, but the CTE's base case is seeded at the immediate LEFT or
    // RIGHT child (position = :position) instead of every immediate child -- everything from
    // there down is that child's own subtree, so this counts "how many associates are in my left
    // leg" / "in my right leg", not a filtered slice of the combined downline.
    @Query(value = """
        WITH RECURSIVE downline(id) AS (
    ...
    // review queue, including freshly-provisioned ones who have not uploaded a document yet --
    // the queue is the admin's to-do list of who still needs verifying. (An earlier revision
    // added an EXISTS(AssociateKycDocument) filter to hide zero-document associates; that hid
