...
public class RootAssociateController {
    ...
    @PostMapping
    public ResponseEntity<CreateRootAssociateResponse> create(@Valid @RequestBody CreateRootAssociateRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(rootAssociateProvisioningService.create(request));
    }
