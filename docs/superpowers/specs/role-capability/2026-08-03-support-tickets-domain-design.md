...
1. **`GET /api/associates/me/support-tickets` stays reachable by an ADMIN token, unguarded.** Confirmed as harmless and consistent with every other `/api/associates/me/*` route in this codebase — no ASSOCIATE-only gate added.
2. **No text search over `subject`/`description` in v1.** Confirmed as deferred, not a v1 gap. Add later if ticket volume ever makes the `status`/`associateId` filters insufficient.
3. **No retention/archival policy for `CLOSED` tickets.** Confirmed — the `status` filter already covers the practical concern; no data-retention rule is invented.
