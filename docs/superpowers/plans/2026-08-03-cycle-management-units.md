...
**Next pending unit:** 2 (no unmet dependencies) or 6 (needs unit 5, now planned — see its plan file — Rank advances to highest qualified tier, never demotes; inserts into `CycleService.close()` between the Matching step and the CLOSED flip, per the sequential-insertion pattern units 3→4→5 established). Unit 11 (screen) still blocked on unit 2.

**Note for unit 5's plan (built):** `CycleService.close()`'s post-rollup insertion point, `CycleCloseResponse`'s current shape (`cycleId, status, legVolumeRowsWritten, newCycleId`), and `LegVolume`'s new `carriedForwardLeft`/`carriedForwardRight` setters were all real and merged going in — unit 5's plan (`docs/superpowers/plans/2026-08-10-cycle-management-matching-income.md`) built directly against them. `CycleService`'s constructor grows to 6 args (`CompensationPlanVersionRepository`, `LedgerEntryRepository` added); unit 6 inherits that shape.
