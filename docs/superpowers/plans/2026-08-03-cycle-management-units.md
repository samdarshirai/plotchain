...
**Next pending unit:** 7/8/9 (7 and 9 depend only on 5, merged; 8/Royalty also unblocked since 6/Rank is merged) — all three insert into `CycleService.close()` at their own point in the flow-step sequence, so sequence them, don't parallelize (file overlap on `CycleService.java`). Unit 11 (screen) is now unblocked (needs 1,2,4 — all merged) but likely best done last alongside/after 10, since its "trigger/monitor" view will look more complete once the full close() pipeline (7-10) is in.
