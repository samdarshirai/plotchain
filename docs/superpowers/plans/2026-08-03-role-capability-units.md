...
| 2 | backend | Admin seeded as single root account via Flyway migration | none | implementing (fixing a real regression found post-implementation — see note below) | `2026-08-13-role-capability-unit-2-admin-seed-migration.md` (own plan, written from scratch — found V16→V18 migration-filename drift and two real `user_id='admin'` fixture collisions the reference plan and spec both missed) | — |
...
| 8 | backend | Associate can submit KYC + view own status | none | planned, **implementation blocked until unit 2 merges** | `2026-08-13-role-capability-unit-8-associate-kyc-submission.md` (own plan — originally clashed with unit 2 on migration number `V18`, renumbered to `V19` at the orchestrator checkpoint) | — |
| 9 | backend | Associate can view own rank progress / reward tiers | none | planned | `2026-08-13-role-capability-unit-9-associate-rank-progress.md` (own plan, written from scratch — no pre-existing plan covers this unit) | — |
...
**Unit 2, in progress 2026-08-13: caught a real regression, not yet merged.** Its implementing agent initially misclassified 3 failing `CycleClose*IntegrationTest` tests as "unrelated in-flight cycle-management work" — verified false by running the same 3 classes against a clean `master` baseline directly (`BUILD SUCCESS`, 4/4 green there, 2 failures + 1 error in the unit-2 worktree). Root cause: the new `V18__seed_founding_admin.sql` migration seeds a permanent `ADMIN`-role associate row into every environment including these integration tests' real Spring context, which shifts count-based assertions off by exactly one (`expected: 8 but was: 9`, `expected: 6L but was: 7L`) and leaves an orphaned `leg_volume` row that `CycleCloseConcurrencyTest.cleanUp` doesn't account for. Sent back to the implementing agent to root-cause and fix within unit 2's own scope (it's a direct consequence of unit 2's own change) rather than merging with an unexplained regression. **Lesson for future units that seed permanent rows into every environment: always diff integration-test results against a clean master baseline before trusting an implementer's "unrelated/pre-existing" classification — don't take it on faith even when the reasoning sounds plausible.**
