...
- No modal/dialog component exists anywhere in this codebase — do not introduce one. Detail drill-down uses `SidePanelComponent` (the original precedent was `SettingsOverviewComponent`'s compensation-history panel; that Company Settings Overview hub screen has since been deleted, but `SidePanelComponent` is unchanged and still in use); Close Cycle is a direct button click with no confirmation dialog, matching how Sales Register's Void action and KYC's reject action both submit directly with no confirm step.
...
- No new component primitives: the design must be expressible using `EditableTableComponent` (table + single `actionTemplate` action column), `InlineBannerComponent` (success/danger banners for the close result and errors), `SidePanelComponent` (the detail drill-down — same component the since-deleted `SettingsOverviewComponent`'s compensation-history panel used; `SidePanelComponent` itself is unchanged and still in use), `BrandButtonComponent` (primary/secondary/danger variants — danger for "Close Cycle", since it's the screen's one consequential, hard-to-undo action). Do not design a data-grid, dialog, toast, or confirmation-modal system.
...
**Interfaces:**
- Consumes: `CycleManagementService.detail(id)` (Task 2), `SidePanelComponent` (`open`, `title`, `(closed)` — `frontend/src/app/shared/components/side-panel/side-panel.component.ts`, same component the since-deleted `SettingsOverviewComponent`'s compensation-history panel used; `SidePanelComponent` itself is unchanged and still in use).
